...
layout (set = 4, binding = 1) uniform UBOParams {
	vec4 _pad0;
	float exposure;
	float gamma;
} uboParams;

layout (set = 4, binding = 2) uniform samplerCube samplerEnv;
...
void main() 
{
	vec3 uvw = vec3(inUVW.x, -inUVW.y,inUVW.z);
	vec3 color = SRGBtoLINEAR(tonemap(textureLod(samplerEnv, uvw, 1.5))).rgb;	
